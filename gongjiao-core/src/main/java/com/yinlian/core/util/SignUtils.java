...
public class SignUtils {
    ...
    public static String signBySoft(String privateKey, String signStr, int modulus)
        ...
        try {
            ...
            byte[] encryptxtArr = RsaUtils.encryptByPrivateKey(Base64Util.decode(privateKey.getBytes("utf-8")), sha1,
                    modulus);

            encryptxtArr = Base64Util.encode(encryptxtArr);
    ...
    public static boolean verifyingSign(String publicKey, String signCipher, String signStr, int modulus)
            throws Exception {
        try {
            byte[] signCipherArr = RsaUtils.decryptByPublicKey(Base64Util.decode(publicKey.getBytes("utf-8")),
            		Base64Util.decode(signCipher.getBytes("utf-8")), modulus);
